...
exports.isMentionedInMessage = (card, userSlug, userGroups = []) => {
	const payload = _.get(card, [ 'data', 'payload' ], {})
	...
	if (_.intersection(alertsGroup.concat(mentionsGroup), userGroups).length > 0) {
		return true
	}
	...
	const userMentionRegExp = new RegExp(`(\\s|^|\\[)[!@]${_.escapeRegExp(userSlug.slice(5))}`)
	if (message.match(userMentionRegExp)) {
		return true
	}

	// Then check if one of the user's groups is mentioned
	const groupMentionRegExp = new RegExp(`(\\s|^|\\[)(!!|@@)(${_.escapeRegExp(userGroups.join('|'))})`)
	...
	const userInMarkerRegExp = new RegExp(`(\\+|^)${_.escapeRegExp(userSlug)}(\\+|$)`)
...
class CardSdk {
     ...
     *
     * @param {import('./types').PartialContract} card - the contract to be created
     *
	   * @returns {Promise<import('./types').ActionResponse>}
     *
     * @example
     * sdk.card.create({
     * 	type: 'thread',
	 ...
	 *
	 * @param {import('./types').PartialContract} fromCard - The card that should be linked from
	 * @param {import('./types').PartialContract} toCard - The card that should be linked to
	...
	async link (fromCard, toCard, verb) {
		...
		const payload = {
			...
			arguments: {
				reason: null,
				properties: {
					slug: `link-${fromCard.id}-${verb.replace(/\s/g, '-')}-${toCard.id}-${uuid()}`,
					tags: [],
					version: '1.0.0',
					links: {},
					requires: [],
	...
	async markAsRead (userSlug, card, userGroups = []) {
		...
		if (typeBase !== 'message' && typeBase !== 'whisper' && typeBase !== 'summary' && typeBase !== 'rating') {
			throw new Error(
		...
		if (exports.isMentionedInMessage(card, userSlug, userGroups)) {
