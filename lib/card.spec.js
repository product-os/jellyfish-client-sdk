...
ava('link throws on invalid link', async (test) => {
	const aTask = {
		id: 'some-task',
		type: 'task@1.0.0'
	}
	...
	await test.throwsAsync(async () => await cardSdk.link(aTask, anOpportunity, 'needs'))
})

ava('link allows links with asterisks', async (test) => {
	const aTask = {
		id: 'some-task',
		type: 'task@1.0.0'
	}
	...
	await test.notThrowsAsync(async () => await cardSdk.link(aTask, anOpportunity, 'was built into'))
})
