...
ava('markAsRead does not update the card if the user is not mentioned', async (test) => {
	await testMarkAsReadMention(test, makeMessageCard('Hello @some-other-user @@some-other-group'), 'user-testuser', [ 'group1' ], false)
})
...
ava('isMentionedInMessage filters for user and group mentions and alerts and 1-to-1 conversations', async (test) => {
	...
	test.true(isMentionedInMessage(Object.assign(makeMessageCard('Hello'), { markers: [ 'user-test-user']}), 'user-test-user'))
	test.true(isMentionedInMessage(Object.assign(makeMessageCard('Hello'), { markers: [ 'org-balena+user-test-user' ] }), 'user-test-user'))
	test.true(isMentionedInMessage(Object.assign(makeMessageCard('Hello'), { markers: [ 'some-other-marker', 'user-test-user+org-balena' ] }), 'user-test-user'))

	// 1-to-1 conversations with similar users are not identified!
	test.false(isMentionedInMessage(Object.assign(makeMessageCard('Hello'), { markers: [ 'user-test-useri '] }), 'user-test-user'))
	test.false(isMentionedInMessage(Object.assign(makeMessageCard('Hello'), { markers: [ 'user-test-user-' ] }), 'user-test-user'))
})
